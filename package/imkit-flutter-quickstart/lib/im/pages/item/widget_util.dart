...
class WidgetUtil {
  ...
  /// 用户头像
  static Widget buildUserPortrait(String path, {bool isTz = false, bool isTs = false}) {
    Widget protraitWidget;
    if (isTs) {
      protraitWidget = Image.asset(
        "assets/images/xiaoxi_tongzhi1.png",
        fit: BoxFit.fill,
        package: 'flutter_rong',
      );
    } else if (isTz) {
      protraitWidget = Image.asset(
        "assets/images/xiaoxi_tongzhi2.png",
        fit: BoxFit.fill,
        package: 'flutter_rong',
      );
    } else if (path == null || path.isEmpty || path == 'null' || path == '') {
      protraitWidget = Image.asset(
  ...
  static Widget buildVoiceRecorderWidget() {
    return Container(
      ...
      child: Container(
        ...
        height: 150,
        child: Image.asset(
          "assets/images/voice_recoder.gif",
          package: 'flutter_rong',
        ),
      ),
    );
